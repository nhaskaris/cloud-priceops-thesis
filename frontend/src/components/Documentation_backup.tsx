...
if response.status_code == 201:
    ...
    print(f"❌ Error: {response.text}")`}
      ...
      {/* Section 4.5: Supporting Extra Model Files */}
      <section id="extra-model-files" style={{ marginBottom: '3rem' }}>
        <h3 style={{ color: '#f1f5f9', fontSize: '1.5rem', borderBottom: '2px solid #334155', paddingBottom: '0.5rem', marginBottom: '1rem' }}>
          Supporting Extra Model Files
        </h3>

        <div style={{ background: '#1e293b', padding: '1.5rem', borderRadius: 8, border: '1px solid #334155', marginBottom: '1.25rem' }}>
          <h4 style={{ color: '#60a5fa', marginTop: 0 }}>When to use</h4>
          <p style={{ color: '#cbd5e1', marginBottom: '0.75rem' }}>
            If your model needs additional artifacts (e.g., tokenizers, embeddings, feature dictionaries), add first-class support in the model registry so the prediction task can load them reliably.
          </p>
        </div>

        <div style={{ background: '#1e293b', padding: '1.5rem', borderRadius: 8, border: '1px solid #334155', marginBottom: '1.25rem' }}>
          <h4 style={{ color: '#60a5fa', marginTop: 0 }}>Implementation steps</h4>
          <ol style={{ color: '#cbd5e1', lineHeight: 1.8, marginBottom: 0 }}>
            <li><strong style={{ color: '#f1f5f9' }}>Add a field in model registry models</strong>: update <code style={{ background: '#0f172a', padding: '0.25rem 0.5rem', borderRadius: 4, color: '#60a5fa' }}>model_registry/models.py</code> with a new <code style={{ background: '#0f172a', padding: '0.25rem 0.5rem', borderRadius: 4, color: '#60a5fa' }}>FileField</code> (e.g., <code style={{ background: '#0f172a', padding: '0.25rem 0.5rem', borderRadius: 4, color: '#60a5fa' }}>extra_asset</code>) and run migrations.</li>
            <li><strong style={{ color: '#f1f5f9' }}>Expose via serializers and API</strong>: include the new field in the serializers and accept it in the create/update endpoints so uploads persist.</li>
            <li><strong style={{ color: '#f1f5f9' }}>Load in prediction task</strong>: update the prediction task in <code style={{ background: '#0f172a', padding: '0.25rem 0.5rem', borderRadius: 4, color: '#60a5fa' }}>model_registry/tasks.py</code> (or the relevant inference module) to read the new artifact when loading the model.</li>
            <li><strong style={{ color: '#f1f5f9' }}>Update client forms</strong>: surface the new upload field in the Contribute UI and ensure multipart submission includes it.</li>
            <li><strong style={{ color: '#f1f5f9' }}>Open a PR</strong>: submit changes (models, migrations, serializers, tasks, UI) to <a href="https://github.com/nhaskaris/cloud-priceops-thesis" style={{ color: '#60a5fa' }}>cloud-priceops-thesis</a> for review.</li>
          </ol>
        </div>
      </section>
